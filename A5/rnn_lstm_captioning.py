...
def temporal_softmax_loss(x, y, ignore_index=None):
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    loss=torch.functional.cross_entropy(x.reshape(-1, x.shape[2]), y.reshape(-1), ignore_index=ignore_index, reduction='sum')/x.shape[0]
    ##########################################################################
    #                             END OF YOUR CODE                           #
    ##########################################################################
...
class CaptioningRNN(nn.Module):
    ...
    def forward(self, images, captions):
        ...
        if self.cell_type == "attn":
            ...
            h0=self.feature_projection(A.mean(dim=(2, 3)))
        else:
            features=self.image_encoder(images)
            h0=self.feature_projection(features)
        word_vectors=self.word_embedding(captions_in)
        if self.cell_type=="attn":
            h, _=self.rnn(word_vectors, h0, A)
        else:
            h, _=self.rnn(word_vectors, h0)
    ...
    def sample(self, images, max_length=15):
        ...
        if(self.cell_type=="attn"):
            ...
            h=self.feature_projection(features_avg)
        ...
        for t in range(max_length):
            ...
            if self.cell_type=="rnn":
                h_next, _=self.rnn(w.unsqueeze(1), h.unsqueeze(0))
                h=h_next.squeeze(1)
            elif self.cell_type=="lstm":
                h_next, (h, c)=self.rnn(w.unsqueeze(1), (h.unsqueeze(0), c.unsqueeze(0)))
                h=h_next.squeeze(1)
            elif self.cell_type=="attn":
                h, c, attn_weights=self.rnn(w, h, c, A)
                attn_weights_all[:, t]=word
        scores=self.output_projection(h)
...
def dot_product_attention(prev_h, A):
    ...
    scores = torch.bmm(prev_h.unsqueeze(1), A_flat)
    scores = scores.squeeze(1)

    scaled_scores = scores / (H ** 0.5)
    attn_weights = torch.softmax(scaled_scores, dim=-1)

    attn_weights = attn_weights.view(N, D_a, D_a)
    attn = torch.bmm(attn_weights.view(N, D_a * D_a, 1), A_flat.transpose(1, 2))  # Shape (N, H, 1)
    attn = attn.squeeze(2)
    ##########################################################################
    #                             END OF YOUR CODE                           #
    ##########################################################################
...
class AttentionLSTM(nn.Module):
    ...
    def step_forward(
        ...
        # Replace "pass" statement with your code
        gates=(
                torch.mm(x, self.Wx) + torch.mm(prev_h, self.Wh) + torch.mm(attn, self.Wattn) + self.b
        )
    ...
    def forward(self, x: torch.Tensor, A: torch.Tensor):
        ...
        for t in range(x.size(1)):
            ...
            h0, c0=self.step_forward(x_t, h0, c0, attn)
            h_states.append(h0)

        hn=torch.stack(h_states, dim=1)  # Shape: (N, T, H)
        ######################################################################
        #                           END OF YOUR CODE                         #
        ######################################################################
        return hn
