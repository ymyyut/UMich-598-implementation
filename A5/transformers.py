...
def generate_token_dict(vocab):
    ...
    #token_dict = {word: idx for idx, word in enumerate(vocab)}
...
def scaled_dot_product_two_loop_single(
    ...
    for i in range(K):
        ...
        w_sum=torch.sum(w.unsqueeze(1)*value, dim=0)
        out[i]=w_sum
    ##############################################################################
    #               END OF YOUR CODE                                             #
    ##############################################################################
...
def scaled_dot_product_two_loop_batch(
    ...
    description in TODO for implementation.
    args:
        query: a Tensor of shape (N to,K, M) where N is the batch size, K is the
    ...
    for n in range(N):
        ...
        k=key[n]
        v=value[n]
        scores=torch.matual(q,k.T)/math.sqrt(M)
...
class SelfAttention(nn.Module):
    def __init__(self, dim_in: int, dim_q: int, dim_v: int):
        ...
        nn.init.uniform_(self.v.weight, -a, a)
        ##########################################################################
        #               END OF YOUR CODE                                         #
        ##########################################################################
...
class MultiHeadAttention(nn.Module):
    def __init__(self, num_heads: int, dim_in: int, dim_out: int):
        ...
        nn.init.uniform_(self.linear.weight, -a, a)
        ##########################################################################
        #               END OF YOUR CODE                                         #
        ##########################################################################
    ...
    def forward(
        ...
        ##########################################################################
        # Replace "pass" statement with your code
        heads=[head(query, key, value, mask) for head in self.attention_heads]
...
def get_subsequent_mask(seq):
    ...
    # Replace "pass" statement with your code
    N, K=seq.shape
    mask=torch.triu(torch.ones(K, K), 1).bool()
...
def position_encoding_simple(K: int, M: int) -> Tensor:
    ...
    y=torch.arange(K) / K
    y=y.unsqueeze(0).expand(M, -1).unsqueeze(0)
    y=y.unsqueeze(0) 
    ##############################################################################
    #               END OF YOUR CODE                                             #
    ##############################################################################
...
def position_encoding_sinusoid(K: int, M: int) -> Tensor:
    ...
    for i in range(M):
        if i%2 == 0:
            y[:, i]=torch.sin(torch.arange(K)/10000 **(i/M))
        else:
            y[:, i]=torch.cos(torch.arange(K)/10000 **((i-1)/M))
    y=y.unsqueeze(0)
    ##############################################################################
    #               END OF YOUR CODE                                             #
    ##############################################################################
