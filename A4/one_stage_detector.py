...
class FCOSPredictionNetwork(nn.Module):
    ...
    def __init__(
        ...
        for i in range(len(stem_channels) - 1):
            stem_cls.append(nn.Conv2d(in_channels, stem_channels[i], kernel_size=3, padding=1))
            stem_cls.append(nn.ReLU())
            stem_box.append(nn.Conv2d(in_channels, stem_channels[i], kernel_size=3, padding=1))
            stem_box.append(nn.ReLU())
            in_channels = stem_channels[i]
        stem_cls.append(nn.Conv2d(in_channels, stem_channels[-1], kernel_size=3, padding=1))
        stem_box.append(nn.Conv2d(in_channels, stem_channels[-1], kernel_size=3, padding=1))
...
class FCOS(nn.Module):
    ...
    def forward(
        ...
        locations_per_fpn_level = get_fpn_location_coords(
            images, pred_cls_logits, self.pred_net.pred_cls
        )
        ######################################################################
        #                           END OF YOUR CODE                         #
        ######################################################################
        ...
        for i in range(images.shape[0]):
            matched_gt_boxes.append(fcos_match_locations_to_gt(locations_per_fpn_level, self.backbone.strides, gt_boxes[i]))
            ...
            matched_gt_deltas.append(
                {
                    level_name: fcos_get_deltas_from_locations(
                        locations, gt_boxes, self.backbone.strides[level_name]
                    )
        ...
        loss_cls = sigmoid_focal_loss(
            pred_cls_logits, matched_gt_boxes[:, :, 4], reduction="none"
        )
        ...
        loss_cls[matched_gt_boxes[:, :, 4] == -1] = 0
        loss_box[matched_gt_boxes[:, :, 4] == -1] = 0
        loss_ctr[matched_gt_boxes[:, :, 4] == -1] = 0
        ######################################################################
        #                            END OF YOUR CODE                        #
        ######################################################################
    ...
    def inference(
        ...
        for level_name in locations_per_fpn_level.keys():
            ...
            level_pred_boxes = fcos_apply_deltas_to_locations(
                level_deltas, level_locations, self.backbone.strides[level_name]
            )
