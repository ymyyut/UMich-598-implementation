...
def svm_loss_vectorized(
    ...
    margins[margins<0]=0
    loss=torch.sum(margins)/X.shape[0]+reg*torch.sum(W*W)
    #############################################################################
    #                             END OF YOUR CODE                              #
    #############################################################################
    ...
    binary[torch.arange(X.shape[0]),y]=-torch.sum(binary,dim=1)
    dW=X.t().mm(binary)/X.shape[0]+2*reg*W
    #############################################################################
    #                             END OF YOUR CODE                              #
    #############################################################################
...
def softmax_loss_vectorized(
    ...
    p=torch.exp(scores)/torch.exp(scores).sum(dim=1, keepdim=True)
    p_log=-torch.log(p[torch.arange(N), y])
    loss=p_log.sum()
    loss/=N
    loss+=reg*torch.sum(W*W)
