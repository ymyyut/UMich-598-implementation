...
def nn_forward_pass(params: Dict[str, torch.Tensor], X: torch.Tensor):
    ...
    hidden0=X.mm(W1)+b1.view(1,-1)
    hidden=hidden0.clamp(min=0)
    scores=hidden.mm(W2)+b2.view(1,-1)
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
...
def nn_forward_backward(
    ...
    # the Softmax classifier loss. When you implement the regularization over W,#
    ...
    loss=-torch.log(p[torch.arange(N),y])
    loss=loss.mean()
...
def nn_train(
    ...
    for it in range(num_iters):
        ...
        #########################################################################
        # Replace "pass" statement with your code
        for params_member in params:
            params[params_member]-=grads[params_member]*learning_rate
        #########################################################################
        #                             END OF YOUR CODE                          #
        #########################################################################
...
def nn_predict(
    ...
    ###########################################################################
    # Replace "pass" statement with your code
    h1 = torch.relu(X.mm(params["W1"]) + params["b1"])
    scores = h1.mm(params["W2"]) + params["b2"]
...
def nn_get_search_params():
    ...
    ###########################################################################
    # Replace "pass" statement with your code
    learning_rates = [50, 10 , 1, 0.5]
    hidden_sizes = [16, 32, 512, 256]
    regularization_strengths = [0, 1e-5, 1e-4, 1e-6]
    learning_rate_decays = [1.0, 0.7, 0.9, 0.8]
    ###########################################################################
    #                           END OF YOUR CODE                              #
    ###########################################################################
