...
class TwoLayerNet(object):
    ...
    def __init__(self, input_dim=3*32*32, hidden_dim=100, num_classes=10,
        ...
        self.paramsp['w2']=weight_scale*torch.randn(hidden_dim, num_classes, device=device, dtype=dtype)
    ...
    def loss(self, X, y=None):
        ...
        ###################################################################
        # Replace "pass" statement with your code
        s=scores-scores.max(dim=1, keepdim=True)[0]
        exp_s=s.exp()
        p=exp_s/exp_s.sum(dim=1, keepdim=True)

        N=X.shape[0]
        loss=-torch.log(p[torch.arange(N), y]).sum()/N
        loss+=self.reg*((w1**2).sum()+(w2**2).sum())

        dscore=p.clone
        dscore[torch.arange(N),y]-=1
        dscore/=N
        ...
        grads['W1'] = X.T.mm(dh1) + 2*self.reg*W1
...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
        ...
        for i in (self.num_layers+1):
            W_name=f'W{i}'
            d_name=f'b{i}'
            self.params[W_name]=weight_scale* torch.randn(dim[i-1], dim[i])
            self.params[d_name]=torch.zeros(dim[i])
        #######################################################################
        #                         END OF YOUR CODE                            #
        #######################################################################
    ...
    def loss(self, X, y=None):
        ...
        for i in self.num_layers:
            ...
            b=self.params[f'b{i}']
            output=input@W+b
            output_relu=output.clamp(min=0)
            cache = (input, W, b, output)
            if self.use_dropout:
                mask=(torch.rand_like(output_relu)<self.dropout_param['p']).float()/self.dropout_param['p']
                output_relu*=mask
                cache+=(mask, )
            caches.append(cache)
            input=output_relu
        ...
        scores=input@W_last+b_last
        caches.append((input, W_last, b_last,  None))
        #################################################################
        #                      END OF YOUR CODE                         #
        #################################################################
        ...
        out_prev, W_last, b_last, _=caches[-1]
        grads[f'W{self.num_layers}']=out_prev.T.mm(dscores)+self.reg * W_last
        grads[f'b{self.num_layers}']=dscores.sum(dim=0)
        dout=dscores.mm(W_last.T)

        for i in range(self.num_layers-1, 0, -1):
            cache=caches[i - 1]
            if self.use_dropout:
                out_prev, W, b, out_linear, mask=cache
                dout *= mask  # dropout反向传播
            else:
                out_prev, W, b, out_linear=cache
            drelu=dout.clone()
            drelu[out_linear <= 0]=0

            grads[f'W{i}']=out_prev.T.mm(drelu) + self.reg * W
            grads[f'b{i}']=drelu.sum(dim=0)
            dout=drelu.mm(W.T)
...
def adam(w, dw, config=None):
    ...
    next_w=w-lr*m_hat/(torch.sqrt(v_hat)+epsilon)
