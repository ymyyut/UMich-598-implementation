...
class Conv(object):
            ...
            for f in range(F):
                for h in range(H_out):
                    for w in range(W_out):
                        h_start=h*stride
                        w_start=w*stride
                        x_slice=x_padded[i, :, h_start:h_start+HH, w_start:w_start+WW]
                        out[i,f,h,w]=torch.sum(x_slice*w[f])+b[f]
        #####################################################################
        #                          END OF YOUR CODE                         #
        #####################################################################
        cache = (x, w, b, conv_param)
    ...
    def backward(dout, cache):
        ...
        dx, dw, db=torch.zero_like(x), torch.zeros_like(w), torch.zeros_like(b)
        x_padded=torch.nn.functional.pad(x, (pad,pad,pad,pad))
        ...
        for n in range(N):
            for f in range(F):
                for h in range(H_out):
                    for w in range(W_out):
                        x_slice=x_padded[n, :, h*stride:h*stride+HH, w*stride:w*stride+WW]
                        dx[n, :, h*stride:h*stride+HH, w*stride:w*stride+WW]+=w[f]*dout[n, f, h, w]
                        dw[f]+=x_slice*dout[n, f, h, w]
            ...
            for c in range(C):
                for h in range(H_out):
                    for w in range(W_out):
                        ...
                        dx[n, c, h_start:h_start+pool_height, w_start:w_start+pool_width]+=mask*dout[n, c, h, w]
        ####################################################################
        #                          END OF YOUR CODE                        #
        ####################################################################
        return dx
...
class ThreeLayerConvNet(object):
    ...
    def loss(self, X, y=None):
        ...
        out1_relu, cache1_relu=ReLU.forward(out1)
        out1_pool, cache1_pool=MaxPool.forward(out1_relu, pool_param)
        N, F, H_out, W_out=out1_pool.shape
        out1_pool=out1_pool.view(N, F*H_out*W_out)
        out2, cache2=Linear.forward(out1_pool, W2, b2)
        out2_relu, cache2_relu=ReLU.forward(out2)
        scores, cache3=Linear.forward(out2_relu, W3, b3)
        ######################################################################
        #                             END OF YOUR CODE                       #
        ######################################################################
        ...
        dout=ReLU.backward(dout, cache2_relu)
        dout, grads['W2'], grads['b2']=Linear.backward(dout, cache2)
        dout=MaxPool.backward(dout.view(N, F, H_out, W_out), cache1_pool)
        dout=ReLU.backward(dout, cache1_relu)
        _, grads['W1'], grads['b1']=Conv.backward(dout, cache1)
    ...
    def __init__(self,
        ...
        for i in range(len(num_filters)):
            if weight_scale=='kaiming':
                weight_scale='relu'
            if weight_scale=='relu':
                weight_initializer=kaiming_initializer
            self.params['W'+str(i+1)]=weight_initializer(C, num_filters[i], K=3, relu=True, device=device, dtype=dtype)
            ...
            if i in max_pools:
                H, W=H//2, W//2
        self.params['W'+str(len(num_filters)+1)]=weight_initializer(C*H*W, num_classes, K=None, relu=False, device=device, dtype=dtype)
        self.params['b'+str(len(num_filters)+1)]=torch.zeros(num_classes, dtype=dtype, device=device)
        ################################################################
        #                      END OF YOUR CODE                        #
        ################################################################
    ...
    def loss(self, X, y=None):
        ...
        for i in range(self.num_layers-1):
            ...
            if self.batchnorm:
                ...
                out, cache=Conv_BatchNorm_ReLU_Pool.forward(out, W, b, gamma, beta, conv_param, pool_param, self.bn_params[i])
            else:
        ...
        dout, grads['W'+str(self.num_layers)], grads['b'+str(self.num_layers)]=Linear.backward(dout, caches.pop())
        for i in range(self.num_layers-1, 0, -1):
            if self.batchnorm:
                dout, grads['W'+str(i)], grads['b'+str(i)], grads['gamma'+str(i)], grads['beta'+str(i)]=Conv_BatchNorm_ReLU_Pool.backward(dout, caches.pop())
            else:
                dout, grads['W'+str(i)], grads['b'+str(i)]=Conv_ReLU_Pool.backward(dout, caches.pop())
...
def find_overfit_parameters():
    ...
    ###########################################################
    # Replace "pass" statement with your co
    weight_scale=1e-2
    learning_rate=1e-3
    ###########################################################
    #                       END OF YOUR CODE                  #
...
def create_convolutional_solver_instance(data_dict, dtype, device):
    ...
    #########################################################
    # Replace "pass" statement with your co
    model=DeepConvNet(input_dims=(3, 32, 32), num_filters=[32, 32, 32], max_pools=[0, 1, 2], batchnorm=True, num_classes=10, weight_scale=1e-2, reg=0.001, weight_initializer=kaiming_initializer, dtype=dtype, device=device)
    solver=Solver(model, data_dict, num_epochs=30, batch_size=50, update_rule=adam, optim_config={'learning_rate':1e-3}, verbose=True)
    solver.train()
    #########################################################
    #                  END OF YOUR CODE                     #
    #########################################################
...
class BatchNorm(object):
    ...
    def backward(dout, cache):
        ...
        dsample_var=(-0.5*dx_hat*(x-sample_mean)*((sample_var+eps)**(-1.5))).sum(dim=0)
        dsample_mean=(-dx_hat/torch.sqrt(sample_var+eps)).sum(dim=0)+dsample_var*(-2*(x-sample_mean)).sum(dim=0)/N
        dx=dx_hat/torch.sqrt(sample_var+eps)+dsample_var*2*(x-sample_mean)/N+dsample_mean/N
        #################################################################
        #                      END OF YOUR CODE                         #
        #################################################################
    ...
    def backward_alt(dout, cache):
        ...
        dgamma=(dout*x_hat).sum(dim=0)
        dbeta=dout.sum(dim=0)
        dx=(dx_hat-torch.mean(dx_hat, dim=0)-(x-sample_mean)*torch.mean(dx_hat*(x-sample_mean), dim=0)/(sample_var+eps))/torch.sqrt(sample_var+eps)
        #################################################################
        #                        END OF YOUR CODE                       #
        #################################################################
